...
namespace OCSServices.Matrixx.Api.Client.Internal.Subscriber
{
    ...
    public class SubscriberModifyOffer : BaseApiOperation, IPostApiOperation<ModifyOfferForSubscriberRequest, MatrixxResponse>
    {
        internal SubscriberModifyOffer(Endpoint endpoint) : base(endpoint)
        {
        }

        public Task<MatrixxResponse> Execute(ModifyOfferForSubscriberRequest payLoad)
        {
            return Post<ModifyOfferForSubscriberRequest, MatrixxResponse>(payLoad);
        }
    }
}
